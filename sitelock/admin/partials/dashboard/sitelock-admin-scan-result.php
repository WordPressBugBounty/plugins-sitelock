...
$upgrade_prompt       = isset($this->wpslp_partner_data['upgrade']['value']['prompt_text']) ? $this->wpslp_partner_data['upgrade']['value']['prompt_text'] : '';
$upgrade_redirect_url = isset($this->wpslp_partner_data['upgrade']['value']['url']) ? $this->wpslp_partner_data['upgrade']['value']['url'] : '';
?>
...
<div id="scan-result" class="simple-box border-grey-light bg-[#fff] mb-6">
    ...
    <?php if (!$sitelock_connection_status || count($sitelock_services) == 0) { ?>
        ...
        <div class="p-0 sm:px-5 min-h-[250px] ">
            ...
            <?php
            $tokenlastKey = array_key_last($sitelock_language_tokens['cloud_services_list']);
        foreach ($sitelock_language_tokens['cloud_services_list'] as $i => $list) {
            ...
            <div class="flex justify-between bg-gradient-to-b from-[#F7F7F7] to-[#FFFFFF] py-2 px-2 sm:px-5">
                ...
                <div class="block sm:flex text-[14px] font-normal leading-[150%] py-4">
                    ...
                    <a target="_blank" rel="noopener noreferrer" class="text-[#2D68C4] underline cursor-pointer ml-1"
                        href="<?php echo esc_url(sitelock_api_url() . '/login?redirect_to=site.settings.wordpress&siteId=' . get_site_identifier()); ?>">
    ...
    <?php } else { ?>
        <div class="p-0 sm:px-5">
            <?php
        if (isset($sitelock_services['advisories'])) {
            unset($sitelock_services['advisories']);
        }
        ...
        $lastKey = array_key_last($sitelock_services);
        foreach ($sitelock_services as $index => $service): ?>
                <?php
            if ($index == 'scanning') {
                $code          = $sitelock_language_tokens['service_title_tokens'][$index];
                $lastScanDate  = $service['lastScanDate'] ?? null;
                $status        = $service['overallStatus'];
                $scanningArray = [];
                if (isset($service['smart_scan'])) {
                    $scanningArray[] = $service['smart_scan'];
                }
                if (isset($service['db_scan'])) {
                    $scanningArray[] = $service['db_scan'];
                }
                if (isset($service['malware_scan'])) {
                    $scanningArray[] = $service['malware_scan'];
                }
                if (isset($service['vulnerability_scan'])) {
                    $scanningArray[] = $service['vulnerability_scan'];
                }
                $featureStatusSortMap;
                uasort($scanningArray, function ($a, $b) use ($featureStatusSortMap) {
                    return ($featureStatusSortMap[$a['status']]) <=> $featureStatusSortMap[$b['status']];
                });
            } elseif ($index == 'backup') {
                if (!isset($service['code'])) {
                    continue;
                }
                $code         = $sitelock_language_tokens['service_title_tokens'][$service['code']];
                $lastScanDate = $service['lastBackupDate'] ?? null;
                $status       = $service['status'];

                if ($service['availability'] != 'upgradable' && $service['status'] != 'unconfigured' && $service['status'] != 'partiallyConfigured' && $service['status'] != 'awaitingFirstBackup') {
                    $fileBackupConfigured = false;
                    $dbBackupConfigured   = false;
                    $filesStatus          = $service['lastBackup']['files']['status'] ?? null;
                    $dbStatus             = $service['lastBackup']['db']['status']    ?? null;
                    foreach ($service['subFeatures'] as $feature) {
                        if ($feature['shortName'] === 'Backup Database' && $feature['isConfigured'] === 1) {
                            $dbBackupConfigured = $feature['isConfigured'];
                        } elseif ($feature['shortName'] === 'Backup Files' && $feature['isConfigured'] === 1) {
                            $fileBackupConfigured = $feature['isConfigured'];
                        }
                    }
                    $showFilesStatus = $fileBackupConfigured ? 'active' : 'inactive';
                    if ($filesStatus === 'error') {
                        $showFilesStatus = 'issuesFound';
                    }
                    if (!$fileBackupConfigured) {
                        $showFilesStatus = 'unconfigured';
                    }

                    $showDbStatus = $dbBackupConfigured ? 'active' : 'inactive';
                    if ($dbStatus === 'error') {
                        $showDbStatus = 'issuesFound';
                    }
                    if (!$dbBackupConfigured) {
                        $showDbStatus = 'unconfigured';
                    }
                    // Get quota in MB, fallback to 0
                    $quotaValue = isset($service['quota']) ? $service['quota'] : 0;
                    $quota      = is_numeric($quotaValue) ? (int) $quotaValue : intval((string) $quotaValue);

                    // Get DB usage in KB, fallback to 0
                    $dbUsageKbValue = isset($service['utilization']['db']['usage_kb']) ? $service['utilization']['db']['usage_kb'] : 0;
                    $dbUsageKb      = is_numeric($dbUsageKbValue) ? (int) $dbUsageKbValue : intval((string) $dbUsageKbValue);

                    // Get file usage in KB, fallback to 0
                    $filesUsageKbValue = isset($service['utilization']['files']['usage_kb']) ? $service['utilization']['files']['usage_kb'] : 0;
                    $filesUsageKb      = is_numeric($filesUsageKbValue) ? (int) $filesUsageKbValue : intval((string) $filesUsageKbValue);

                    // Total usage in KB and MB
                    $usageKb = $dbUsageKb + $filesUsageKb;
                    $usageMb = $usageKb / 1000;

                    // Calculate percentage used
                    $percentageStorageUsed = ($quota > 0) ? ($usageMb / $quota) * 100 : 0;

                    // Format percentage
                    $formattedPercentageStorageUsed = ($percentageStorageUsed >= 1)
                        ? number_format(floor($percentageStorageUsed), 0)
                        : number_format($percentageStorageUsed, 2);

                    // Convert total storage MB to GB if available
                    $totalStorageInMb = $quota; // or another value if defined separately
                    $totalStorageInGb = ($totalStorageInMb) ? ($totalStorageInMb / 1000) : 0;
                }
            } else {
                if (!isset($service['code'])) {
                    continue;
                ...
                $status       = $service['status'];
                if ($index == 'patchman') {
                    $lastGoodScanDate = $service['lastGoodScanDate'] ?? null;
                }
            }
            $url = $index == 'scanning' ? $index : $service['code'];

            ?>
                <div class="<?php echo esc_attr($lastKey == $index ?? 'not-last-child:') ?>border-b border-[#EEEEEE] px-3 sm:px-0">
                    <div class="grid grid-cols-12 sm:grid-cols-12 gap-4 py-[10px]">
                        ...
                        <div class="col-span-12 sm:col-span-4 c1 flex items-center justify-start ">
                            <div>
                                <button type="button" class="toggle mr-3" data-id="<?php echo esc_attr($url); ?>">
                                    <span><span></span> </span>
                                </button>
                            </div>
                            <?php if ($service['upgradable']) { ?>
                                ...
                                <?php endif; ?>
                            <?php } elseif (($url == 'patchman' || $url == 'waf' || $url == 'backup') && isset($service['showConfigure']) && $service['showConfigure'] && isset($service['hideDetails']) && !$service['hideDetails']) { ?>
                                <a class="title" target="_blank"
                                    href="<?php echo esc_url(sitelock_api_url() . '/sites/' . esc_attr($sitelock_site_info['id']) . '/wizard/' . esc_attr($sitelock_language_tokens['service_setup_url_tokens'][$url])); ?>">
                                    <?php echo esc_html($code); ?>
                                </a>
                            <?php } elseif (($url == 'patchman' || $url == 'waf' || $url == 'backup') && $service['status'] == 'unconfigured' || (isset($service['showConfigure']) && $service['showConfigure'])) { ?>
                                <a class="title" target="_blank"
                                    href="<?php echo esc_url(sitelock_api_url() . '/sites/' . esc_attr($sitelock_site_info['id']) . '/wizard/' . esc_attr($sitelock_language_tokens['service_setup_url_tokens'][$url])); ?>">
                                    <?php echo esc_html($code); ?>
                                </a>
                            ...
                            <?php } else { ?>
                                <a class="title" target="_blank"
                                    href="<?php echo esc_url(sitelock_api_url() . '/sites/' . esc_attr($sitelock_site_info['id']) . '/' . esc_attr($sitelock_language_tokens['service_url_tokens'][$url])); ?>">
                                    <?php echo esc_html($code); ?>
                                </a>
                            <?php }
                            ?>
                        ...
                        <div class="col-span-6 sm:col-span-3 c2 flex items-center justify-start">
                            <div>
                                ...
                                <div class="flex items-center justify-start">
                                    <?php if (isset($service['code']) && $service['code'] == 'advisories') { ?>
                                        <div class="pr-2">
                                            <span class="icon circle w-5 <?php echo esc_attr($sitelock_language_tokens['service_status_map'][$status]); ?>"> </span>
                                        </div>
                                        <!-- Text -->
                                        <span class="status"><?php echo isset($service['score']) && isset($sitelock_language_tokens['service_risk_score_token'][$service['score']]) ? esc_html($sitelock_language_tokens['service_risk_score_token'][$service['score']]) : esc_html(isset($sitelock_language_tokens['service_status_tokens']['notEnrolled']) ? $sitelock_language_tokens['service_status_tokens']['notEnrolled'] : ''); ?> </span>
                                    <?php } elseif (isset($service['code']) && $service['code'] == 'backup' && ($status == 'nearQuota' || $status == 'overQuota')) { ?>
                                        <span class="icon circle w-5 mr-2 <?php echo esc_attr($formattedPercentageStorageUsed < 80 ? esc_attr($sitelock_language_tokens['service_status_map']['active']) : ($formattedPercentageStorageUsed > 80 ? esc_attr($sitelock_language_tokens['service_status_map']['nearQuota']) : esc_attr($sitelock_language_tokens['service_status_map']['unconfigured']))); ?>"></span>
                                        <span class="status"><?php echo esc_html(number_format($formattedPercentageStorageUsed, 0)) . '% Full'; ?></span>
                                    <?php } else { ?>
                                        ...
                                        <!-- Text -->
                                        <span class="status"><?php echo esc_html($sitelock_language_tokens['service_status_tokens'][$status]); ?> </span>
                                    <?php } ?>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="col-span-6 sm:col-span-3 c3 flex items-center text-left">
                            <?php if ($lastScanDate) { ?>
                                <div>
                                    ...
                                    <div class="flex-1 <?php echo esc_attr($url == 'spam_scan' ? 'email-last-scan' : 'last-scan') ?>">
                                        <?php echo esc_html($lastScanDate) ?>
                                    </div>
                                </div>
                            <?php } ?>
                        </div>
                        ...
                        <div class="col-span-4 sm:col-span-2 c4 flex items-center text-left">
                            ...
                            <?php } elseif (($url == 'patchman' || $url == 'waf' || $url == 'backup') && (isset($service['showConfigure']) && $service['showConfigure'] && isset($service['hideDetails']) && !$service['hideDetails'])) { ?>
                                <div class="relative w-full relative-wrapper">
                                    <a href="<?php echo esc_url_raw(sitelock_api_url()) . '/sites/' . esc_attr($sitelock_site_info['id']) . '/wizard/' . esc_attr($sitelock_language_tokens['service_setup_url_tokens'][$url]); ?>"
                                        ...
                                        <span class="dropdown-arrow border border-l-[#A2BEEB]">
                                            <img src="<?php echo esc_url(plugin_dir_url(__DIR__) . '../images/down-arrow-blue.svg'); ?>"
                                                alt="<?php echo esc_attr__('Down Arrow', 'sitelock-wordpress-plugin'); ?>" class="w-3 arrow-toggle" />
                                        </span>
                                    </a>
                                    <div class="dropdown base-dropdown" style="display:none;">
                                        <span><a class="link" target="_blank" rel="noopener noreferrer"
                                                href="<?php echo esc_url(sitelock_api_url() . '/sites/' . esc_attr($sitelock_site_info['id']) . '/wizard/' . esc_attr($sitelock_language_tokens['service_setup_url_tokens'][$url])); ?>">Set
                                                Up</a></span>
                                        <span><a class="link" target="_blank" rel="noopener noreferrer"
                                                href="<?php echo esc_url(sitelock_api_url() . '/sites/' . esc_attr($sitelock_site_info['id']) . '/' . esc_attr($sitelock_language_tokens['service_url_tokens'][$url])); ?>">Details</a></span>
                                    </div>
                                </div>
                            <?php } elseif (($url == 'patchman' || $url == 'waf' || $url == 'backup') && $service['status'] == 'unconfigured' || (isset($service['showConfigure']) && $service['showConfigure'])) { ?>
                                <a target="_blank"
                    ...
                    <div id="<?php echo esc_attr($url); ?>" class="collapsed">
                        <div
                            class="<?php echo esc_attr('grid grid-cols-1 md:grid-cols-12 gap-1 sm:gap-4 mb-3 ' . ($url == 'scanning' ? 'pr-2 sm:pr-0 pl-[28px]' : '')); ?>">
                            <?php if ($url == 'scanning') { ?>

                                <?php if (isset($service['db_scan'])) { ?>
                                    <div
                                        class="md:col-span-6 order-<?php echo esc_attr(array_search('db_scan', array_column($scanningArray, 'code')) + 1); ?>">
                                        <?php include plugin_dir_path(__FILE__) . 'sitelock-admin-security-scan/sitelock-security-scan-db.php'; ?>
                                    </div>
                                <?php } ?>

                                <?php if (isset($service['smart_scan'])) { ?>
                                    <div
                                        class="md:col-span-6 order-<?php echo esc_attr(array_search('smart_scan', array_column($scanningArray, 'code')) + 1); ?>">
                                        <?php include plugin_dir_path(__FILE__) . 'sitelock-admin-security-scan/sitelock-security-scan-file.php'; ?>
                                    </div>
                                <?php } ?>

                                <?php if (isset($service['vulnerability_scan'])) { ?>
                                    <div
                                        class="md:col-span-6 order-<?php echo esc_attr(array_search('vulnerability_scan', array_column($scanningArray, 'code')) + 1); ?>">
                                        <?php include plugin_dir_path(__FILE__) . 'sitelock-admin-security-scan/sitelock-security-scan-vulnerability.php'; ?>
                                    </div>
                                <?php } ?>

                                <?php if (isset($service['malware_scan'])) { ?>
                                    <div
                                        class="md:col-span-6 order-<?php echo esc_attr(array_search('malware_scan', array_column($scanningArray, 'code')) + 1); ?>">
                                        <?php include plugin_dir_path(__FILE__) . 'sitelock-admin-security-scan/sitelock-security-scan-malware.php'; ?>
                                    </div>
                                <?php } ?>

                            <?php } elseif ($url == 'ssl_scan' || $url == 'advisories') { ?>
                                <div class="md:col-span-12">
                                    <p class="mx-5 sm:mx-10 text-[14px] leading-[150%] mb-4 font-normal text-[#6A6A6A]">
                                        <?php echo esc_html($sitelock_language_tokens['cloud_services_description'][$url]); ?>
                                    </p>
                                </div>
                            <?php } elseif ($url == 'backup') { ?>
                                <?php if ($service['availability'] == 'upgradable' || $service['status'] == 'unconfigured' || $service['status'] == 'partiallyConfigured' || $service['status'] == 'awaitingFirstBackup') { ?>
                                    <div class="md:col-span-12">
                                        <p class="mx-5 sm:mx-10 text-[14px] leading-[150%] mb-4 font-normal text-[#6A6A6A]">
                                        ...
                                        <div class="px-5 md:px-0">
                                            <div class="flex justify-between items-center border-b border-[#EEEEEE]">
                                                <h4 class="my-2 scan-result-title text-[#000000]">
                                                    <?php echo esc_html($totalStorageInGb) . ' ' . esc_html($sitelock_language_tokens['var']['storage']); ?>
                                                </h4>
                                                <div>
                                                    <p class="mt-2 mb-1 scan-result-data text-[#333333] flex items-center">
                                                        <span
                                                            class="icon circle w-5 mr-2 <?php echo esc_attr($formattedPercentageStorageUsed < 80 ? $sitelock_language_tokens['service_status_map']['active'] : ($formattedPercentageStorageUsed > 80 ? $sitelock_language_tokens['service_status_map']['nearQuota'] : $sitelock_language_tokens['service_status_map']['unconfigured'])) ?>"></span>
                                                        <?php echo esc_html(number_format($formattedPercentageStorageUsed, 2)) . '% full' ?>
                                                    </p>
                                                    <?php if ($percentageStorageUsed > 80) { ?>
                                                    <?php if ($this->wpslp_partner_data['upgrade'] === '[default]'): ?>
                                                            <a href="<?php echo esc_url(admin_url() . 'admin.php?page=sitelock-upgrade'); ?>"
                                            ...
                                            <div class="flex justify-between items-center border-b border-[#EEEEEE]">
                                                ...
                                                <p class="my-2 scan-result-data text-[#333333] flex items-center">
                                                    <span
                                                        class="icon circle w-5 mr-2 <?php echo esc_attr($sitelock_language_tokens['service_status_map'][$showFilesStatus]); ?> "></span><?php echo esc_html($sitelock_language_tokens['service_status_tokens'][$showFilesStatus]); ?>
                                                </p>
                                            </div>
                                            <?php if (isset($service['lastBackup']['files']) && $service['lastBackup']['files']['created_at']) { ?>
                                                <div class="flex justify-between items-center border-b border-[#EEEEEE]">
                                                    <h4 class="my-2 scan-result-title text-[#000000]">
                                                    ...
                                                    <p class="my-2 scan-result-data text-[#333333] flex items-center last-scan">
                                                        <?php echo esc_html(isset($service['lastBackup']['files']) && $service['lastBackup']['files']['created_at'] ? $service['lastBackup']['files']['created_at'] : ''); ?>
                                                    </p>
                                                </div>
                                            <?php } ?>
                                            <div
                                                class="flex justify-between items-center <?php echo esc_attr(!isset($service['lastBackup']['db']) || !$service['lastBackup']['db']['created_at'] ? 'not-last-child:' : '') ?>border-b border-[#EEEEEE]">
                                                <h4 class="my-2 scan-result-title text-[#000000]">
                                                    <?php echo esc_html($sitelock_language_tokens['var']['databaseBackup']); ?>
                                                </h4>
                                                <p class="my-2 scan-result-data text-[#333333] flex items-center">
                                                    <span
                                                        class="icon circle w-5 mr-2 <?php echo esc_attr($sitelock_language_tokens['service_status_map'][$showDbStatus]); ?>"></span><?php echo esc_html($sitelock_language_tokens['service_status_tokens'][$showDbStatus]); ?>
                                                </p>
                                            </div>

                                            <?php if (isset($service['lastBackup']['db']) && $service['lastBackup']['db']['created_at']) { ?>
                                                <div class="flex justify-between items-center not-last-child:border-b border-[#EEEEEE]">
                                                    ...
                                                    <p class="my-2 scan-result-data text-[#333333] flex items-center last-scan">
                                                        <?php echo esc_html(isset($service['lastBackup']['db']) && $service['lastBackup']['db']['created_at'] ? $service['lastBackup']['db']['created_at'] : ''); ?>
                                                    </p>
                                                </div>
                                            <?php } ?>
                                ...
                                <?php } ?>
                            <?php } elseif ($url == 'spam_scan') { ?>
                                <div class="<?php echo ($service['availability'] !== 'upgradable') ? 'md:col-span-6' : 'md:col-span-12'; ?>">
                                ...
                                <?php if ($service['availability'] !== 'upgradable'): ?>
                                <div class="md:col-span-6">
                                    <div class="px-5 md:px-0">
                                        <?php if ($lastScanDate): ?>
                                            <div class="flex justify-between items-center border-b border-[#EEEEEE]">
                                        ...
                                        <div class="flex justify-between items-center not-last-child:border-b border-[#EEEEEE]">
                                            ...
                                            <p class="my-2 scan-result-data text-[#333333]"><?php echo esc_html($service['frequency']); ?></p>
                                        </div>
                                    </div>
                                </div>
                                <?php endif ?>
                            <?php } elseif ($url == 'patchman') { ?>
                                <div class="<?php echo ($service['availability'] !== 'upgradable') ? 'md:col-span-6' : 'md:col-span-12'; ?> mx-5 sm:mx-10 ">
                                    ...
                                    <?php 
                                    $upgradable = isset($service['upgradable']) ? $service['upgradable'] : false;
                                    $status = isset($service['status']) ? $service['status'] : null;
                                    $showConfigure = isset($service['showConfigure']) ? $service['showConfigure'] : false;
                                    $showScanNowButton = $status !== 'failedBilling' && !$upgradable && !$showConfigure;
                                    ?>
                                    <?php if ($showScanNowButton) { 
                                        $nextAvailableScanValue = false; 
                                        if(isset($service['nextAvailableScan']['value']) && !empty($service['nextAvailableScan']['value']) && ($service['nextAvailableScan']['value'] !== 'pending' || $service['nextAvailableScan']['value'] !== 'NA')) {
                                            $nextAvailableScanValue = true; 
                                        } ?>
                                        <div>
                                            <button data-type="<?php echo esc_attr($url); ?>" <?php if ($nextAvailableScanValue)
                                            ...
                                            <?php if (!empty($service['nextScheduledScan']['value'])): ?>
                                                <p
                                                    class="<?php echo !empty($service['nextScheduledScan']['value']) ? 'my-2' : ''; ?> text-[14px] text-pretty">
                                                    <?php echo esc_html($sitelock_language_tokens['var']['automaticSchedule']); ?> <span
                                                        class="scan-result-title text-[#000000] schedule-scan"><?php echo esc_html($service['nextScheduledScan']['value']); ?></span>
                                                </p>
                                            <?php endif; ?>
                                            <?php if (isset($service['nextAvailableScan']['value']) && !empty($service['nextAvailableScan']['value']) && $service['nextAvailableScan']['value'] !== 'pending' && $service['nextAvailableScan']['value'] !== 'NA' && empty($service['nextScheduledScan']['value'])): ?>
                                                <p
                                                    class="<?php echo !empty($service['nextAvailableScan']['value']) ? 'my-2' : ''; ?> text-[14px] text-pretty">
                                                    <?php echo esc_html($sitelock_language_tokens['var']['nextAvailableScan']); ?> <span
                                                        class="scan-result-title text-[#000000] schedule-scan"><?php echo esc_html($service['nextAvailableScan']['value']); ?></span>
                                                </p>
                                        ...
                                        <?php if ($lastGoodScanDate) { ?>
                                            <div class="flex justify-between items-center border-b border-[#EEEEEE]">
                                                ...
                                                <p class="my-2 scan-result-data text-[#333333] last-scan">
                                                    <?php echo esc_html($lastGoodScanDate); ?>
                                                </p>
                                            </div>
                                        <?php } ?>
                                        <div class="flex justify-between items-center border-b border-[#EEEEEE]">
                                            <h4
                                                class="my-2 scan-result-title <?php echo esc_attr(isset($service['lastScan']['num_vulnerable']) && $service['lastScan']['num_vulnerable'] > 0 ? 'text-red' : 'text-[#000000]'); ?> ">
                                                <?php echo esc_html($sitelock_language_tokens['var']['vulnerabilitiesFound']); ?>
                                            </h4>
                                            <p
                                                class="my-2 scan-result-data <?php echo esc_attr(isset($service['lastScan']['num_vulnerable']) && $service['lastScan']['num_vulnerable'] > 0 ? 'text-red' : 'text-[#333333]'); ?>">
                                                <?php echo esc_html(isset($service['lastScan']['num_vulnerable'])                              && $service['lastScan']['num_vulnerable'] ? $service['lastScan']['num_vulnerable'] : 0); ?>
                                            </p>
                                        </div>
                                        <div class="flex justify-between items-center border-b border-[#EEEEEE]">
                            ...
                            <?php } elseif ($url == 'waf') { ?>
                                ...
                                <?php } else { ?>
                                    ...
                                    <div class="md:col-span-6">
                                        <div class="px-5 md:px-0">
                                            <div class="w-full <?php echo esc_attr($service['status'] === 'unconfigured' ? 'not-last-child:border-b' : 'border-b'); ?> border-[#EEEEEE] text-[0px]">
                                                ...
                                                <p class="w-1/2 inline-flex align-top my-2 scan-result-data text-[#333333]">
                                                    <span class="icon circle w-5 mr-2 align-middle <?php echo esc_attr($sitelock_language_tokens['service_status_map'][$service['status']]); ?>"></span>
                                                    <?php echo esc_html($sitelock_language_tokens['service_status_tokens'][$service['status'] === 'ok' ? 'active' : $service['status']] ?? ''); ?>
                                                </p>
                                            </div>
                                        </div>
                                        ...
                                        <?php if ($service['status'] === 'ok') { ?>
                                            <div class="px-5 sm:px-10 md:px-0">
                                                <div class="w-full <?php echo esc_attr($service['statuses']['routingSL'] ? 'border-b' : 'not-last-child:border-b') ?> border-[#EEEEEE] text-[0px]">
                                                    ...
                                                    <p class="w-1/2 inline-flex align-top my-2 scan-result-data text-[#333333]">
                                                        <?php if ($service['statuses']['routingSL']) { ?>
                                            ...
                                            <?php if ($service['statuses']['routingSL']) { ?>
                                                ...
                                                <div class="px-5 sm:px-10 md:px-0">
                                                    <div class="w-full border-b border-[#EEEEEE] text-[0px]">
                                                        ...
                                                        <p class="w-1/2 inline-flex align-top my-2 scan-result-data text-[#333333] items-center">
                                                            <?php if (!empty($service['statuses']['trueSpeed'])) { ?>
                                                ...
                                                <div class="px-5 sm:px-10 md:px-0">
                                                    <div class="w-full not-last-child:border-b border-[#EEEEEE] text-[0px]">
                                                        ...
                                                        <p class="w-1/2 inline-flex align-top my-2 scan-result-data text-[#333333] items-center">
                                                            <?php if (!empty($service['statuses']['trueShield'])) { ?>
                                                                <span class="icon circle w-5 mr-2 <?php echo esc_attr($sitelock_language_tokens['service_status_map']['enabled']); ?>"></span>
                                                                <?php echo esc_html($sitelock_language_tokens['var']['enabled']); ?>
                                                            <?php } else { ?>
                                                                <span class="icon circle w-5 mr-2 <?php echo esc_attr($sitelock_language_tokens['service_status_map']['disabled']); ?>"></span>
            ...
            document.querySelectorAll('.schedule-scan').forEach(function(el) {
                ...
                if (rawText) {
                    ...
                    if (diffInSeconds < 60) {
                        ...
                        displayText = `${diffInHours} hour${diffInHours !== 1 ? 's' : ''}, ${remainingMinutes} minute${remainingMinutes !== 1 ? 's' : ''}`;
                    } else if (diffInDays === 1) {
                        const remainingHour = diffInHours % 24;
                        displayText = `${diffInDays} day, ${remainingHour} hour${remainingHour !== 1 ? 's' : ''}`;
                    } else if (diffInDays <= 90) {
                        const remainingHours = diffInHours % 24;
                        displayText = `${diffInDays} day${diffInDays !== 1 ? 's' : ''}, ${remainingHours} hour${remainingHours !== 1 ? 's' : ''}`;
                    } else {
                        displayText = date ? date.toLocaleDateString('en-US', {
                            year: 'numeric',
                            month: 'short',
