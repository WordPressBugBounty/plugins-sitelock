<div class="<?php echo esc_attr($tokenlastKey == $i ? 'not-last-child:border-b' : 'border-b') ?> border-[#EEEEEE] px-3 sm:px-0">
    <div class="grid grid-cols-12 gap-2 md:gap-3 py-[10px]">
        <!-- Plus/minus toggle button -->
        <div class="c1 flex items-center col-span-5">
            <div>
                <button type="button" class="toggle mr-3" data-id="<?php echo esc_attr($list) ?>">
                    <span><span></span> </span>
                </button>
            </div>
            <h3 href="#" class="heading block sm:flex items-center w-full gap-2">
                <?php
                $service_title = $sitelock_language_tokens['service_title_tokens'][$list];
// Only show FREE for spam_scan and ssl_scan
$show_free = in_array($list, ['spam_scan', 'ssl_scan']) && !$sitelock_connection_status;
?>
        ...
        <div class="flex items-center col-span-3">
                <button type="button" class="toggle-learn-more learn-more-box w-[112px] px-4 text-[14px] text-center py-[6px] bg-[#F6F9FE] text-[#083C8C] border-blue rounded cursor-pointer" data-id="<?php echo esc_attr($list); ?>">
    ...
    <div id="<?php echo esc_attr($list) ?>" class="collapsed">
        <div class="grid grid-cols-12 gap-4 mb-4">
            <?php if ($list === 'scanning') : ?>
                 <div
            ...
            <?php else : ?>
                <div class="col-span-12">
                    <p class="mx-10 text-[14px] leading-[150%] mb-4 font-normal text-[#6A6A6A]">
                        <?php echo esc_html($sitelock_language_tokens['cloud_services_description'][$list]); ?>
                    </p>
