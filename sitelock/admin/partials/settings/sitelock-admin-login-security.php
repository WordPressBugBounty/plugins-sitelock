...
<div id="login-security" class="hidden simple-box border-[#DDDDDD] bg-[#fff] mb-5 p-8 min-h-full xl:min-h-[500px]">
  <div class="mb-6 w-full xl:w-[595px] box-title text-[30px]">
    ...
    ?>
    <div class="w-full">
        <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>" id="login-security-form">
            ...
            <div class="security-options">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-2">
                            <div class="mb-4">
                                ...
                                <div class="block mt-5">
                                    ...
                                    $sitelock_login_security_allowed_roles = ['administrator', 'editor', 'author', 'contributor', 'shop_manager'];

                                    foreach ($wp_roles->roles as $sitelock_login_security_role_slug => $sitelock_login_security_role_info) {
                                        $sitelock_login_security_checked = in_array($sitelock_login_security_role_slug, $sitelock_two_fa_settings['mandatory_roles']) ? 'checked' : '';
                                    ?>
                                        <div class="mb-3">
                                            <div class="flex items-center">
                                                <input
                                                    type="checkbox"
                                                    id="sitelock_2fa_role_<?php echo esc_attr($sitelock_login_security_role_slug); ?>"
                                                    name="sitelock_2fa_settings[mandatory_roles][]"
                                                    value="<?php echo esc_attr($sitelock_login_security_role_slug); ?>"
                                                    <?php echo esc_attr($sitelock_login_security_checked); ?>
                                                    <?php echo !in_array($sitelock_login_security_role_slug, $sitelock_login_security_allowed_roles) ? 'disabled' : ''; ?> />

                                                <label
                                                    for="sitelock_2fa_role_<?php echo esc_attr($sitelock_login_security_role_slug); ?>"
                                                    class="ml-3 <?php echo !in_array($sitelock_login_security_role_slug, $sitelock_login_security_allowed_roles) ? 'cursor-not-allowed' : 'cursor-pointer'; ?>">
                                                    <h2 class="tab-content-field-title <?php echo !in_array($sitelock_login_security_role_slug, $sitelock_login_security_allowed_roles) ? '!text-[#0000006B]' : ''; ?>">
                                                        <?php echo esc_html($sitelock_login_security_role_info['name']); ?>
                                                        <?php echo !in_array($sitelock_login_security_role_slug, $sitelock_login_security_allowed_roles) ? '<span>*</span>' : ''; ?>
                                                    </h2>
                                                </label>
                                            </div>
                                        </div>
                ...
                <!-- 2FA Disable Confirmation Modal -->
                <?php
                $sitelock_2fa_modal_id            = '2fa-disable-confirmation';
                $sitelock_2fa_title               = $sitelock_language_tokens['login_security_list_settings']['2fa_disable_modal']['title'];
                $sitelock_2fa_warning_text        = $sitelock_language_tokens['login_security_list_settings']['2fa_disable_modal']['description'];
                $sitelock_2fa_message_text        = $sitelock_language_tokens['login_security_list_settings']['2fa_disable_modal']['message'];
                $sitelock_2fa_show_list           = true;
                $sitelock_2fa_list_items          = $sitelock_language_tokens['login_security_list_settings']['2fa_disable_modal']['disableFactors'];
                $sitelock_2fa_show_input          = true;
                $sitelock_2fa_input_label         = $sitelock_language_tokens['login_security_list_settings']['2fa_disable_modal']['confirmationLabel'];
                $sitelock_2fa_confirm_button_id   = 'disable-confirmation-button';
                $sitelock_2fa_confirm_button_type = 'button';
                            ...
                            <div class="flex items-baseline mb-4">
                                <div class="number w-[80px] rounded">
                                    <input type="number" name="sitelock_login_lockout_max_attempts"
                                        value="<?php echo esc_attr($sitelock_login_lockout_max_attempts); ?>" min="3" class="input-number h-[36px]" />
                                    <span class="switch"><span></span></span>
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="flex items-baseline mb-4">
                                <div class="number w-[80px] rounded">
                                    <input type="number" name="sitelock_login_lockout_duration"
                                        value="<?php echo esc_attr($sitelock_login_lockout_duration); ?>" min="5" class="input-number h-[36px]" />
                                    <span class="switch"><span></span></span>
                                </div>
...
<script>
    jQuery(document).ready(function($) {
        ...
        // Custom method: reset time < duration
        $.validator.addMethod("lessThanDuration", function(value, element) {
            var duration = parseInt($("input[name='sitelock_login_lockout_duration']").val(), 10);
            var resetTime = parseInt(value, 10);

            // Only validate if both fields have numeric values
            if (!isNaN(duration) && !isNaN(resetTime)) {
                return resetTime <= duration;
            }
            return true; // skip if not numbers
        }, "Reset Time must be less than or equal to Lockout Duration");

        $("#login-security-form").validate({
        rules: {
            sitelock_login_lockout_max_attempts: {
                required: function() {
                    return $('#login-lockout').hasClass('expanded');
                },
                digits: true,
                min: 3
            },
            ...
            sitelock_login_lockout_reset_time: {
                required: function() {
                    return $('#login-lockout').hasClass('expanded');
                },
                digits: true,
                min: 0,
                lessThanDuration: true // <-- custom rule applied
            }
        },
        messages: {
            sitelock_login_lockout_max_attempts: {
                ...
                digits: "Digits only",
                min: "Must be greater than or equal to 3"
            },
            sitelock_login_lockout_duration: {
            ...
            sitelock_login_lockout_reset_time: {
                required: "Lockout Reset Time is required",
                digits: "Digits only",
                min: "Must be greater than or equal to 0",
                lessThanDuration: "Must be less than or equal to Lockout Duration"
            }
        }
    });

    $(document).on('click', '#login-security-submit', function(e) {

        if ($('#login-lockout').hasClass('expanded')) {

            if ($("#login-security-form").valid()) {
                $("#login-security-form")[0].submit();
            } else {
                $('html, body').animate({ scrollTop: $('.login-lockout-setting').offset().top }, 'slow');
            }
